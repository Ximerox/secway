...
#[Layout('admin.layout')]
#[Title('Signaturblock bearbeiten')]
class SignatureEdit extends Component
{
    use WithFileUploads;
    ...
    public function mount(?SignatureTemplate $template = null): void
    {
        ...
        if ($template && $template->exists) {
            ...
            $this->text_body = (string) $template->text_body;
            $this->active = $template->active;
            $this->priority = (int) $template->priority;
            $this->direction = $template->direction;
            $this->sender_mode = $template->sender_mode;
            $this->sender_users = (string) $template->sender_users;
            $this->sender_group_id = (string) $template->sender_group_id;
            $this->sender_exclude = (string) $template->sender_exclude;
            $this->recipient_include = (string) $template->recipient_include;
            $this->recipient_exclude = (string) $template->recipient_exclude;
            $this->valid_from = $template->valid_from?->format('Y-m-d') ?? '';
            $this->valid_until = $template->valid_until?->format('Y-m-d') ?? '';
            $this->on_applied = $template->on_applied;
            $this->on_not_applied = $template->on_not_applied;
        } else {
    ...
    {
        try {
            return \Illuminate\Support\Facades\Cache::remember('graph_groups_list', 600, function () {
                $map = [];
    ...
    public function save()
    {
        $this->validate([
            ...
            'name.required' => 'Bitte einen Namen für den Signaturblock vergeben.',
            'html.required' => 'Der Signaturblock ist leer.',
        ...
        $t = $this->templateId ? SignatureTemplate::find($this->templateId) : new SignatureTemplate;
        ...
        if ($t->sender_mode === 'group' && $t->sender_group_id) {
            $this->resolveGroupMembers($t->sender_group_id);
        }

        session()->flash('ok', 'Signaturblock „'.$t->name.'" gespeichert.');

        // Nach dem ersten Speichern auf die Bearbeiten-URL wechseln (saubere Adresse)
        if ($wasNew) {
            return $this->redirectRoute('admin.signatures.edit', ['template' => $t->id], navigate: true);
        }
    }
    ...
    public function sendTest(SignatureRenderer $renderer): void
    {
        ...
        try {
            ...
            session()->flash('ok', 'Testmail an '.$this->test_to.' versendet (Signaturblock mit Daten von '.$user->display_name.').');
        } catch (Throwable $e) {
    ...
    public function updatedUpload(): void
    {
        $this->validate(['upload' => 'image|max:2048'], [
            ...
            'upload.max' => 'Maximal 2 MB — Bilder sollten klein sein, sie hängen an jeder Mail.',
        ]);
        ...
        SignatureImage::create(['original_name' => $name, 'path' => $path, 'mime' => $mime, 'size' => $size]);

        $this->reset('upload');
        session()->flash('ok', 'Bild hochgeladen — über „In Editor einfügen" übernehmen.');
    }
    ...
    public function deleteImage(int $id): void
    {
        ...
        $img->delete();
        session()->flash('ok', 'Bild gelöscht.');
    }
    ...
    public function saveQr(): void
    {
        ...
        $qr = $this->qrEditId ? SignatureQrCode::find($this->qrEditId) : new SignatureQrCode;
        $qr->fill(['label' => trim($this->qr_label), 'text' => $this->qr_text, 'size' => $this->qr_size])->save();

        $this->qrEditId = null;
        session()->flash('ok', 'QR-Code „'.$qr->label.'" gespeichert.');
    }

    public function deleteQr(int $id): void
    {
        SignatureQrCode::findOrFail($id)->delete();
        session()->flash('ok', 'QR-Code gelöscht.');
    }
